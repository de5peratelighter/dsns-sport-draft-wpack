...
const TABLE_START_PROTOCOL_RETRACTABLE_RESULTS_HEAD = (item) => {
    return new TableRow({
        children: [
...
const TABLE_START_PROTOCOL_RETRACTABLE_RESULTS_ROW = (item) => {
    return new TableRow({
        children: [
            new TableCell(cellObject({ text: item.set, width: 6 })),
            new TableCell({
...
export const TABLE_START_PROTOCOL_RETRACTABLE_RESULTS = (items) => {
    return [
        ...items.map((data) => {
            return new Table({
                rows: [
                    ...TABLE_CATEGORY_RECORDS_ROW(data.head), // Додаємо нові рядки перед головкою таблиці
                    TABLE_START_PROTOCOL_RETRACTABLE_RESULTS_HEAD(data.head),
                    ...data.rows.map((item) => TABLE_START_PROTOCOL_RETRACTABLE_RESULTS_ROW(item)),
                ],
                layout: TableLayoutType.FIXED,
                width: {
                    size: 100,
